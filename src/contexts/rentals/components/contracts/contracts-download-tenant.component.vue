...
const fetchData = async () => {
  try {
    ...
    const ownerResponse = await tenantApiServices.getAllOwners();
    const foundOwner = ownerResponse.data.find((owner) =>
      owner.vehicles.includes(request.value.car_id)
    );

    const userResponseOwner = await tenantApiServices.getUserById(foundOwner.owner_id);
